#### 2014-11-27
#
#  Forest Plot for reference comparision
#      PASS
#
...
### Best
irBest <- boot(hosp.dat , statistic = ir.best , R = 1000, best = 82)

###Best
  ResultsEQVAS[i, 1] <- irBest$t0
ResultsEQVAS[i, 2:3] <- boot.ci(irBest, type='perc')$perc[4:5]
ResultsEQVAS[i, 4:6] <- deltaConf(hosp.dat$eqvas0, hosp.dat$eqvas1, 82)
...
### Best
irBest <- boot(hosp.dat , statistic = ir.best , R = 1000, best = 0.90)

###Best
  ResultsEQ5D[i, 1] <- irBest$t0
ResultsEQ5D[i, 2:3] <- boot.ci(irBest, type='perc')$perc[4:5]
ResultsEQ5D[i, 4:6] <- deltaConf(hosp.dat$eq0, hosp.dat$eq1, 0.90)
...
tiff(filename = "ForestPlot20141127.tif",
...
#############
plot.new()
plot.window(ylim=c(0, 23), xlim=c(60, 100) )
axis(1, lwd = 3, cex.axis = 1.5, font = 2)
## general reference
segments(y0 = 0.5, y1 = 22, x = 81.83084, x1 = 81.83084, col = 'grey', lwd = 2, lty = 3)
...
axis(1, lwd = 3, cex.axis = 1.5, font = 2)
## general reference
segments(y0 = 0.5, y1 = 22, x = 77.82797, x1 = 77.82797, col = 'grey', lwd = 2, lty = 3)
